...
public final class NotaryEvents {
    @SubscribeEvent public static void tick(TickEvent.PlayerTickEvent e){
        ...
        if(BossConfig.notary().enabled()&&NotaryProgression.evidence(d,id)&&d.uniqueInt(id,"notary_defeated")==0&&d.uniqueInt(id,"notary_bell_issued")==0){
            if(p.getInventory().contains(new ItemStack(WardItems.FINAL_WITNESS_BELL.get())))d.setUniqueInt(id,"notary_bell_issued",1);
            else{NotaryProgression.giveBell(p);if(d.uniqueInt(id,"notary_bell_issued")!=0)WardHud.send(p,"FINAL WITNESS // A bell without a tongue has appeared among your things. It bears a notice addressed to you.",WardHud.Mood.GLITCH);}
        }
    }
