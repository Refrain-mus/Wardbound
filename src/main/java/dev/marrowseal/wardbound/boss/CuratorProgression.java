...
public final class CuratorProgression {
    ...
    public static void giveWrit(ServerPlayer p){ItemStack item=new ItemStack(WardItems.ARCHIVE_WRIT.get());boolean delivered=p.getInventory().contains(item)||MasterInvocationRelics.giveOrProtectedDrop(p,item);if(delivered&&p.getServer()!=null)LockData.get(p.getServer()).setUniqueInt(p.getUUID(),"curator_writ_issued",1);}
