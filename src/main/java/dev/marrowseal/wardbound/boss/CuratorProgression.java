...
public final class CuratorProgression {
    ...
    public static void giveWrit(ServerPlayer p){ItemStack item=new ItemStack(WardItems.ARCHIVE_WRIT.get());if(!p.getInventory().contains(item) && !p.getInventory().add(item))p.drop(item,false);}
