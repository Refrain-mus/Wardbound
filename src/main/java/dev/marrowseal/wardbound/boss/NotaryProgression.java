...
public final class NotaryProgression {
    ...
    public static void giveBell(ServerPlayer p){ItemStack bell=new ItemStack(WardItems.FINAL_WITNESS_BELL.get());boolean delivered=p.getInventory().contains(bell)||MasterInvocationRelics.giveOrProtectedDrop(p,bell);if(delivered&&p.getServer()!=null)LockData.get(p.getServer()).setUniqueInt(p.getUUID(),"notary_bell_issued",1);}
