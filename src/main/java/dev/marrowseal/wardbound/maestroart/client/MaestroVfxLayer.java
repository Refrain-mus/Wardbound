...
public final class MaestroVfxLayer extends GeoRenderLayer<MaestroArtEntity> {
    private final Map<MaestroArtEntity,State> states=new WeakHashMap<>();
    private static final class State {
        int revision=-1,tick=-1,budget;float age;
        final Map<String,Vec3> trails=new HashMap<>();
    }
    public MaestroVfxLayer(GeoRenderer<MaestroArtEntity> renderer){super(renderer);}
    @Override public void preRender(PoseStack pose,MaestroArtEntity entity,BakedGeoModel model,RenderType type,MultiBufferSource buffers,VertexConsumer buffer,float partial,int light,int overlay){
    ...
    private void particle(MaestroArtEntity e,State s,SimpleParticleType type,Vec3 p,Vec3 velocity){
        if(p==null||s.budget--<=0)return;
        e.level().addParticle(type,p.x,p.y,p.z,velocity.x,velocity.y,velocity.z);
    }
    private void effect(MaestroArtEntity e,State s,MaestroVfxTimeline.Cue cue){
    ...
    @Override public void render(PoseStack pose,MaestroArtEntity e,BakedGeoModel model,RenderType type,MultiBufferSource buffers,VertexConsumer buffer,float partial,int light,int overlay){
